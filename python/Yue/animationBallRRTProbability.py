...
class ballRRT():
    ...
    def __init__(self, start, goal, randArea, obstacleList, ballRadius, maxCollisionCheckIncrement, maxSamples):
        """
        Setting Parameter
        ...
        self.start=Node(start[0],start[1], goal)
        self.end=Node(goal[0],goal[1], goal)
    ...
    def Planning(self, animation=True):
        ...
        if self.LineCollisionCheck(self.start, self.end, obstacleList): # no collision between start and goal
            pdb.set_trace() #breakpoint
            ...
            # pdb.set_trace() #breakpoint
            [interMNode, noNewNodeAdded] = self.giveIntermediateNode(self.start, self.end, obstacleList)
        ...
        for i_RRTIteration in range(self.maxSamples):
            [rnd, rndNode] = self.sampleBallRRT(ballCenterNode_Loop, ballRadius_Loop, obstacleList)
            # Find nearest node
            ...
            # expand tree
            if self.LineCollisionCheck(nearestNode, rndNode, obstacleList): # no collision between both nodes
                rndNode.parent = nind
                self.nodeList.append(rndNode)
                newNode = rndNode
            else: # collision between two nodes
                [interMNode, noNewNodeAdded] = self.giveIntermediateNode(nearestNode, rndNode, obstacleList)
                if noNewNodeAdded:
                    continue
            ...
            # pdb.set_trace() #breakpoint
            # if self.LineCollisionCheck(newNode, self.end, obstacleList): # no collision between new node and goal
            #    print("Goal!!")
            #    break
            # else: # there is collision between two nodes
                # pdb.set_trace() #breakpoint
                # goalNode = Node()
            [newNode2, noNewNodeAdded] = self.giveIntermediateNode(newNode, self.end, obstacleList)
            if not noNewNodeAdded: # added some node
                ...
                if newNode2.distanceToTarget < nearestNode.distanceToTarget:
                    ...
                    ballRadius_Loop = ballRadius_Loop * 5
            else:
                ballCenterNode_Loop = nearestNode
                # ballRadius_Loop = ballRadius_Loop + ballRadius_Loop
                ballRadius_Loop = ballRadius_Loop * 5
    ...
    def giveIntermediateNode(self, firstNode, secondNode, obstacleList):
        ...
        for i_step in range(int(stepNum)-1):
            ...
            crrtNode = Node(crrt[0], crrt[1], [self.end.x, self.end.y])
            # pdb.set_trace() #breakpoint
            if self.__CollisionCheck(crrtNode, obstacleList): # no collision
                ...
                nodeBeforeCllsn = Node(beforeCllsn[0], beforeCllsn[1], [self.end.x, self.end.y])
    ...
    def sampleBallRRT(self, centerNode, ballRadius, obstacleList):
        ...
        while(not validSampling):
            ...
            sampledPointX = np.random.uniform(max(self.minrand,centerNode.x - ballRadius), min(self.maxrand,centerNode.x + ballRadius), 1)
            sampledPointY = np.random.uniform(max(self.minrand,centerNode.y - ballRadius), min(self.maxrand,centerNode.y + ballRadius), 1)
            sampledNode = Node(sampledPointX[0], sampledPointY[0], [self.end.x, self.end.y])

            ifCollisionFreeWithObstacles = self.__CollisionCheck(sampledNode, obstacleList)
            ifInsideBall = LA.norm([sampledPointX - centerNode.x, sampledPointY - centerNode.y]) <= ballRadius
            validSampling = ifCollisionFreeWithObstacles and ifInsideBall 
...
if __name__ == '__main__':
    ...
    print 'BallRRT calculation computing time: ' + str(durationBallRRT.total_seconds()) + 'seconds'
